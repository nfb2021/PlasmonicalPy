...
def csv_to_img(
    ...
    try:
        ...
        print(
            f'The specified file "{csv_file}" was not found in the current workign directory. Program is terminated.'
        )
        exit()
...
def make_all_pngs(
    subdirs: Optional[bool] = True,
    out_format: Optional[str] = "png",
    remove_old_imgs: Optional[str] = False,
) -> None:
    ...
    def get_matching_csv_files() -> List:
        files = natsorted(
            [
                x
                for x in os.listdir()
                if x.endswith(".csv") and "spectrum" not in x and "Meas" in x
            ]
        )
        refs = natsorted(
            [
    ...
    if subdirs == True:
        sub_dirs = [
            os.path.join(root, x)
            for x in os.listdir()
            if os.path.isdir(x) and "Field" in x
        ]
        dirs += sub_dirs
    ...
    for dir in dirs:
        ...
        for match in matches:
            meas, ref = match
            csv_to_img(
                csv_file=meas, csv_reference=ref, out_format=out_format, show_init=False
            )

        os.chdir(root)


def make_all_pngs_no_ref(
    subdirs: Optional[bool] = True,
    out_format: Optional[str] = "png",
    remove_old_imgs: Optional[str] = False,
) -> None:
    ...
    def get_blank_ref(csv_file: str) -> str:
        df = pd.read_csv(
            csv_file, delimiter=" ", header=None, index_col=False
        ).to_numpy()
...
def runner():
    ...
    io = input(
        'Do you want to keep the default paramaters for this action (subdirs = True, out_format = "png", remove_old_imgs = False)? [y/n]: '
    )
    if io == "n":
