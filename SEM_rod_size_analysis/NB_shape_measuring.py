...
bar_plot_dict = {
    "G6": ("red", "darkred", r"$\gamma$6"),
    "D2": ("blue", "darkblue", r"$\delta$2"),
    "D4": ("magenta", "purple", r"$\delta$4"),
}
...
def get_corr_factors(csv_file, specs):
    ...
    if check == False:
        print(
            f"There are no corresponding correction factors based on your input {spec_used}. Try one of the following: {idxs}. Aborting."
        )
        exit()
...
def get_distances(lst_a, lst_b, m=2):
    ...
    distances_out = _distances_out[
        abs(_distances_out - np.mean(_distances_out)) < m * np.std(_distances_out)
    ]
    coords_out = [
        _coords_out[idx] for idx, x in enumerate(_distances_out) if x in distances_out
    ]
...
def get_attr(sample_name):
    try:
        ...
        return "darkred", "darkred", "Unknown label"
...
def plot_single_results():
    ...
    dict_all = {}
    specs = input(
        "Specify the accel. voltage [kV], working distance and magnification (separated by commas, no spaces): "
    ).split(",")
    x_factor, x_std, y_factor, y_std = get_corr_factors(
        "NB_XL30_calibration.csv", specs
    )
    ...
    for i in trange(len(rods), desc="Evaluating SEM images..."):
        ...
        contours, hierarchies = cv.findContours(
            binary_otsu, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_NONE
        )
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                ax0.set_ylim(
                    int(ymin - 0.05 * img_height), int(ymax + 0.05 * img_height)
                )
                ...
                try:
                    ax2.hist(
                        lengths_nm,
                        bins=int(np.sqrt(len(lengths_nm))),
                        color=l_color,
                        edgecolor="black",
                    )
                except ValueError:
                ...
                try:
                    ax3.hist(
                        widths_nm,
                        bins=int(np.sqrt(len(widths_nm))),
                        color=w_color,
                        edgecolor="black",
                    )
                except ValueError:
                ...
                if f"{sample}uCcm-2_widths [nm]" not in dict_all.keys():
                    ...
                    dict_all[f"{sample}uCcm-2_widths [nm]"] = np.concatenate(
                        (dict_all[f"{sample}uCcm-2_widths [nm]"], np.array(widths_nm)),
                        axis=0,
                    )
                ...
                if f"{sample}uCcm-2_lengths [nm]" not in dict_all.keys():
                    ...
                    dict_all[f"{sample}uCcm-2_lengths [nm]"] = np.concatenate(
                        (
                            dict_all[f"{sample}uCcm-2_lengths [nm]"],
                            np.array(lengths_nm),
                        ),
                        axis=0,
                    )
