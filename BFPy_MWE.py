from BFPy import (
    Analysis,
)  # import the BFPy module. Pay attention to its location on your drive
import os
import numpy as np
import shutil
...
if __name__ == "__main__":
    ...
    dirs = [
        os.path.join(root, x) for x in os.listdir() if os.path.isdir(x) and "Field" in x
    ]  # all directories containing the data
    dirs.append(root)
    ...
    for dir in dirs:  # iterating over all directories
        ...
        pngs = [
            ...
            for x in os.listdir()
            if x.endswith(".png")
            and x.endswith("original.png")
            and not x.startswith("csv")
        ]  # all corrected gray scale uint8 bfp images, need to be created priorly
        for p, png in enumerate(pngs):
            ...
            try:
                ...
                masked_background, name_masked_background = file.mask_background(
                    file.img_non_uint8,
                    lower_thres=int(0.05 * np.amax(file.img_non_uint8)),
                    upper_thres=int(0.95 * np.amax(file.img_non_uint8)),
                    show_hist=False,
                    show=False,
                )  # mask background of orignal
                (
                    masked_background_superres,
                    name_masked_background_superres,
                ) = file.get_super_res(
                    masked_background, model="EDSR_x3.pb", blur=True, show=False
                )  # create superresolution version of masked background

                file.make_false_color_img(masked_background, name_masked_background)
                file.make_false_color_img(
                    masked_background_superres, name_masked_background_superres
                )

                # now, radial intensity analysis
                superres = [
                    x for x in os.listdir() if x.endswith("superres.png")
                ]  # on superres images
                originals = [
                    x for x in os.listdir() if x.endswith("original.png")
                ]  # on normal images
                ...
                #         shutil.copy(oo, os.path.join("Radial_Intensities", oo))
                    ...
                    print(
                        "Neither original images nor images with superesolution were found. Aborting."
                    )
                    exit()
                ...
                for img in os.listdir():
                    ...
                    for thrs in thresholds:
                        file.get_radial_slice_intensities(
                            threshold=thrs, only_rad_plot=True, show=False
                        )
                        file.get_radial_slice_intensities(
                            threshold=thrs, only_rad_plot=False, show=False
                        )

            except:
                print(
                    "\t\tAn error occured. Skipping this file and moving it into quarantine."
                )
