...
class CsvToImg:
    ...
    def __init__(self, csv_file: str, csv_reference: str, out_format: Optional[str] = 'png', show_init: Optional[bool] = False) -> None:
        ...
        try:
            ...
            print(f'The specified file "{self.csv_file}" was not found in the current workign directory. Program is terminated.')
            exit()
        ...
        self.img_uint8: np.ndarray = np.array(self.arr).astype('uint8')     # 8-bit grayscale image
        self.img_float: np.ndarray = np.array(self.arr).astype('float64')   # grayscale image of float type, for further manipulations
...
class Image:
    ...
    def __init__(self, img: str):
        if os.path.isfile(img):
            ...
            if os.path.isfile(self.corresponding_csv):
                ...
                refs = [x for  x in os.listdir() if x.endswith('.csv') and 'spectrum' not in x and 'Ref' in x]
                    ...
                    if meas_split == ref_split:
                        ...
                        if meas_num == ref_num:
                            ...
                            try:
                                ...
                                print(f'The specified file "{self.csv_meas}" was not found in the current working directory. Program is terminated.')
                                exit()
                            ...
                            try:
                                ...
                                print(f'The specified reference "{self.csv_ref}" was not found in the current workign directory. Continuing without reference.')
            ...
            self.blur: np.ndarray = cv.GaussianBlur(self.gray, (5,5), cv.BORDER_DEFAULT)                                    # blurred version obtained by convolution with 5x5 Gaussian kernel
            _, self.binary_otsu = cv.threshold(self.blur, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)            # binary obtaiend by Otsu's method
            _, self.binary_median = cv.threshold(self.blur, ndimage.median(self.blur), 255, cv.THRESH_BINARY)   # binary obtaiend by taking median as threshold
    ...
    def get_contours(self, binary: np.ndarray, show: Optional[bool] = True) -> Tuple[np.ndarray, float]:
        ...
        contours, hierarchies = cv.findContours(binary, mode = cv.RETR_EXTERNAL, method = cv.CHAIN_APPROX_NONE)
        if show == True:
    ...
    def get_coords_from_contours(self, contour_list: np.ndarray) -> Tuple[List[int], List[int]]:
        ...
        xx, yy = [], []
        contours = contour_list[0]  # only takes one found contour into account, thats why the longest contour is stored in a list as single element
        for px in contours:
            xx.append(px[0][0])
            yy.append(px[0][1])
    ...
    def mask_background(self, img_gray: np.ndarray, lower_thres: Optional[int] = 12, upper_thres: Optional[int] = 242, pad: Optional[float] = 0.1, show: Optional[bool] = False, save: Optional[bool] = True, show_hist: Optional[bool] = False) -> Tuple[np.ndarray, str]:
        ...
        if show_hist == True:
            ...
            ax0.axvline(lower_thres, color = 'red', label = f'lower threshold: {lower_thres}')
            ax0.axvline(upper_thres, color = "blue", label = f'upper threshold: {upper_thres}')
            ...
            if choice == 'n':
                ...
                lower_thres, upper_thres = int(choice.split(',')[0]), int(choice.split(',')[1])
                if np.amin(gray) <= lower_thres < upper_thres and upper_thres <= np.amax(gray):
        ...
        _x_diff, _y_diff = np.abs(np.max(xx) - np.min(xx)), np.abs(np.max(yy) - np.min(yy))
        ...
        masked_background = np.zeros([int((1 + 2 * pad) * background_length), int((1 + 2 * pad) * background_length)])


        for y in range(len(gray)):
            for x in range(len(gray[0])):
                if np.min(xx) <= x <= np.max(xx) and np.min(yy) <= y <= np.max(yy):
                    masked_background[y - np.min(yy) + int(pad * background_length)][x - np.min(xx) + int(pad * background_length)] = gray[y][x]
    ...
    def get_super_res(self, img_gray: np.ndarray, model: Optional[str] = 'EDSR_x3.pb', blur: Optional[bool] = False, show: Optional[bool] = False, save: Optional[bool] = True) -> Tuple[np.ndarray, str]:
        ...
        if blur == True:
            img_gray_superres = cv.GaussianBlur(img_gray_superres, (3,3), cv.BORDER_DEFAULT)

        if show == True:
            fig = plt.figure(2021)
            ax = fig.add_subplot(111)
            _img_gray_superres = self.normalize_for_cbar(img_gray_superres)
            masked_array = np.ma.array(_img_gray_superres, mask=(_img_gray_superres == 0))
            im = ax.imshow(masked_array, cmap=cmap)
            plt.axis('off')
            divider = make_axes_locatable(ax)
            cax = divider.append_axes('right', size='5%', pad=0.05)
...
class FourierCorrelationAnalysis(Image): 
    ...
    def get_correlation_coeff_alternative(self, img1: np.ndarray, img2: np.ndarray) -> np.ndarray:
...
class Analysis(FourierCorrelationAnalysis):
    ...
    def get_radial_intensities(self, only_rad_plot: Optional[bool] = True, show: Optional[bool] = True, save: Optional[bool] = True) -> Tuple[List, List]:
        ...
        lines = [([int(cx), x], [int(cy), y]) for (x,y) in zip(X, Y)] # all lines connecting the center coordinate to each coordinate of the identified contour
        line_lens = []
        ...
        for (color, line) in zip(colors, lines):
            xx, yy = line
            line_lens.append((np.sqrt(np.abs(xx[1] - xx[0])**2 + np.abs(yy[1] - yy[0])**2)))        # length of each line
            intensities.append(np.sum(profile_line(self.gray, (xx[0], yy[0]), (xx[1], yy[1]))))     # sum of pixel values along line
        ...
        _intensities = [(a / b) for (a, b) in zip(intensities, line_lens)]                          # normalization with line length
        ...
        degs = [x*deg for x, xx in enumerate(intensities)]                                  # calculate angle of each pixel of contour
        degs_p = [np.angle((xx - cx) +1j * (yy - cy)) + np.pi for (xx, yy) in zip(X, Y)]    # calculate angle of each pixel of contour
